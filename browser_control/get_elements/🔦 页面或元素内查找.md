...
**说明**  
loc 元组是指 selenium 定位符，例：(By.ID, '****')。下同。`ele('****', index=2)` 和 `eles('****')[1]` 结果一样，不过前者会快很多。
...
由于 shadow-root 不能跨级查找，链式操作非常常见，所以设计了一个简写：`sr`，功能和 `shadow_root` 一样，都是获取元素内部的 `ShadowRoot`。多级 shadow-root 链式操作示例：以下这段代码，可以打印浏览器历史第一页，可见是通过多级 shadow-root 来获取的。
